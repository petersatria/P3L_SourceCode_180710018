    ...
    <v-container>
        <v-card class="mt-10 pa-10">
                <div class="d-flex flex-row my-5">
                    <v-btn icon color="indigo" light @click="$router.push('/dokter');">
            ...
            <v-card-text>
                <v-container>
                    ...
                    <table>
                        ...
                        <tr>
                            <td>Tanggal Lahir </td>
                            <td>: {{customer.tgl_lahir_cust}}</td>
                        </tr>
                    </table>
                    <h2 class="mb-3 mt-10">Keluhan : </h2>
                    ...
                    <v-form :v-model="valid" lazy-validation ref="form">
                        <v-textarea label="Keluhan" v-model="formKeluhan.keluhan"  :rules="requiredRule"></v-textarea>
                    </v-form>

                    <h2 class="mb-3 mt-10">Data Transaksi : </h2>
                    <h3 >Daftar Produk :</h3>
...
export default {
    data() {
        return{
            ...
            urlKeluhan : this.$apiUrl + '/Transaksi/keluhan/',
            urlCashier : this.$apiUrl + '/Transaksi/cashier/',
            headersDetilProduk : [
                {text : "No", value : "no"},
    ...
    methods : {
        ...
        getDataRiwayat(kode_cust){
            let uri = this.urlKeluhan+kode_cust;
            console.log(uri);
            this.$http.get(uri).then(response => {
                    this.riwayat = response.data.message
            }).catch(error =>{                   
                this.snackbar = true;               
                this.message = error;               
                this.snackabrColor = 'error';      
        ...
        checkIsMedis(){
            ...
            this.request.append('keluhan', this.formKeluhan.keluhan);
            let uri = this.urlKeluhan+this.kode_transaksi;
            this.$http.post(uri,this.request).then(response =>{            
                this.snackbar = true;
                this.message = response.data.message;
                if(!response.data.error){
                    if(this.detilPerawatan.length > 0){
                        this.$router.push('/Dokter/pemeriksaan/beautician');
                    }else{
                        this.statusCashier();
                    }
                }else{
        ...
        statusCashier(){
            this.$http.post(this.urlCashier+this.kode_transaksi).then(response =>{            
                this.snackbar = true;
                this.message = response.data.message;
                if(!response.data.error){
                    this.snackabrColor = 'success'; 
