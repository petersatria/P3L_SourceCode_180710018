<template>
    <v-container>
        <v-card class="mt-10 pa-10">
            <v-card-title
            class="font-weight-black d-flex justify-center mb-5 text">Laporan Produk Terlaris</v-card-title>
            <v-card-text>
        ...
        submit(){
            for (let index = 0; index < this.bulans.length; index++) {
             if(this.bulans[index] == this.bulan){
                 window.open(this.$apiUrl+'laporan/produkLaris?bulan='+(index+1)+'&tahun='+this.tgl);
                }
            }
