...
Class NotaProduk extends REST_Controller{
    ...
    function index_get(){
        ...
        $isMember = $produk[0]->isMember;
        if ($isMember == 1) {
            $member = $this->get_member($produk[0]->id_member);
            $member_nama = $member[0]->nama;
            $member_telp = $member[0]->no_telp;
        }
        else{
            $member_nama = '-';
            $member_telp = '-';
        ...
        $pdf->Cell(140,6,'Telepon : '.$member_telp,0,0,'L');
    ...
    public function test_get(){
        return $this->response($this->NotaProdukModel->get('PR-020320-03'));
    }
