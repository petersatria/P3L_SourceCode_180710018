...
class JenisHewan extends REST_Controller
{
	...
	{
        if($id==null){
            $response = $this->JenisHewanModel->get();
			return $this->returnData($response, false);
        }else{
			$response = $this->JenisHewanModel->search($id);
			return $this->returnData($response,false);
		}
	}

	public function byString_get($nama = null){
		$response = $this->JenisHewanModel->searchByString($nama);
		return $this->returnData($response, false);
	}
	...
	public function index_post($id = null)
	{
		...
		if($id != null){
			...
			$response = $this->JenisHewanModel->update($ukuran);
		}else{
			$ukuran = new data();
			$ukuran->keterangan = $this->post('keterangan');
			$ukuran->created_by = $this->PegawaiModel->getIdPegawai($this->post('created_by'));
			$response = $this->JenisHewanModel->store($ukuran);
		}
	...
	public function delete_post($id=null){
		...
		$ukuran->updated_by = $this->PegawaiModel->getIdPegawai($this->post('updated_by'));
		$ukuran->id = $id;
		$response = $this->JenisHewanModel->delete($ukuran);
		return $this->returnData($response['msg'], $response['error']);
	}
